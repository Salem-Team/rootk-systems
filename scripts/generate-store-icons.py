...
def main() -> None:
    ...
    foreground = fit_logo(logo, 432, pad_ratio=0.18)
    ...
    for folder, size in densities.items():
        ...
        fg = fit_logo(logo, size, pad_ratio=0.18)
    ...
    for folder, (w, h) in splash_sizes.items():
        ...
        mark = fit_logo(logo, min(w, h) // 3, pad_ratio=0.12)
        x = (w - mark.width) // 2
        y = (h - mark.height) // 2
    ...
    ios_mark = fit_logo(logo, 720, pad_ratio=0.1)
