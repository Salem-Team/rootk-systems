...
export function TaskCompletionEvidenceDialog({
  ...
  const showInvalidMedia =
    evidence.touched && evidence.requireMedia && !mediaOk;
  const notesProgress = Math.min(1, notesLen / EVIDENCE_NOTES_MIN);

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[min(94dvh,920px)] flex-col gap-0 overflow-hidden p-0 sm:max-w-lg">
        <div
          aria-hidden
        ...
        <DialogHeader className="relative shrink-0 border-b border-border/50 px-4 pb-3 pt-1 sm:px-5">
          <motion.div
        ...
        <motion.div
          ...
          className="relative min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-4 py-3.5 sm:px-5"
        >
          <div className="rounded-xl border border-border/70 bg-gradient-to-br from-muted/45 via-muted/20 to-transparent px-3.5 py-3">
            ...
            <ul className="mt-2.5 space-y-2 text-[13px]">
              ...
              {evidence.requireMedia ? (
                <li className="flex items-start gap-2.5">
                  <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-md bg-primary/10 text-primary">
                    <ImagePlus className="h-3 w-3" aria-hidden />
                  </span>
                  <span>
                    <span className="font-medium">{t("workEvidence.reqMedia")}</span>
                    <span className="mt-0.5 block text-[11px] text-muted-foreground">
                      {t("workEvidence.reqMediaHint")}
                    </span>
                  </span>
                </li>
              ) : null}
          ...
          {showMedia ? (
            <div
              className={cn(
                "space-y-2",
                showInvalidMedia && "rounded-xl ring-2 ring-destructive/40"
              )}
            >
              ...
              <TaskMediaPicker
                drafts={evidence.mediaDrafts}
                onChange={evidence.setMediaDrafts}
              />
            </div>
          ) : null}
        ...
        <DialogFooter className="relative shrink-0 gap-2 border-t border-border/50 px-4 pb-[max(0.85rem,env(safe-area-inset-bottom))] pt-3 sm:gap-2 sm:px-5">
          <Button
            type="button"
            variant="outline"
